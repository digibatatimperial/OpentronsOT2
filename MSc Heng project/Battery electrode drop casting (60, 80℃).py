from opentrons import protocol_api
from opentrons.types import Point, Location
import math

metadata = {
    "protocolName": "Battery Electrode Slurry Drop Casting: Volume Screen RT + Heating",
    "author": "DIGIBAT Lab",
    "apiLevel": "2.15",
}


def run(protocol: protocol_api.ProtocolContext):

    # ============================================================
    # USER SETTINGS — CHANGE THESE BEFORE EACH RUN
    # ============================================================

    # Run this protocol separately for 60, 70, and 80 °C.
    # The protocol first drop-casts at room temperature, pauses for RT pre-drying,
    # then heats the Heater-Shaker module to this temperature.
    CASTING_TEMPERATURE_C = 37
    DRYING_TEMPERATURE_C = 80

    # Total slurry volume per electrode spot. One spot is generated for each volume.
    # These correspond to the rows in your experiment table.
    DROPLET_VOLUMES_UL = [100, 100, 100, 100, 100]

    # If you want triplicates, do NOT force 15 large droplets onto a small 65 x 50 mm foil.
    # Recommended: keep REPLICATES_PER_VOLUME = 1, and repeat on three separate foils/runs.
    REPLICATES_PER_VOLUME = 1

    # Usable foil area referenced from A1 of the custom 1536 coordinate labware.
    FOIL_X_MM = 70.0
    FOIL_Y_MM = 70.0

    # Dispense height above the foil/reference surface.
    Z_HEIGHT_MM = 0.1

    # Aspiration height above the bottom of the slurry tube.
    SOURCE_ASPIRATE_HEIGHT_MM = 1.0

    # Pipetting parameters adapted from the ADEL paper SI.
    # ADEL SI reports aspirate 10 µL/s, dispense 19.2 µL/s, blowout 270 µL/s.
    # Start with blowout 100 µL/s if worried about disturbing the droplet, then increase if tip retains slurry.
    ASPIRATE_RATE_UL_S = 10
    DISPENSE_RATE_UL_S = 19.2
    BLOWOUT_RATE_UL_S = 100      # change to 270 if following SI strictly and droplet shape is not disturbed
    DELAY_AFTER_PIPETTING_S = 3

    # Optional offsets if A1 is not exactly the desired foil top-left corner.
    X_SHIFT_MM = 0.0
    Y_SHIFT_MM = 0.0

    # ============================================================
    # MODULES
    # ============================================================
    hs4 = protocol.load_module("heaterShakerModuleV1", 4)

    electrode_plate = hs4.load_labware(
        "digibat_1536_wellplate_1000ul",
        label="DIGIBAT 1536 Coordinate Reference"
    )

    # ============================================================
    # LABWARE
    # ============================================================
    rack20_6 = protocol.load_labware(
        "digibat_20ml_8_tube_rack",
        6,
        label="20 mL Tube Rack for Battery Slurry"
    )

    tips1000_8 = protocol.load_labware(
        "opentrons_96_tiprack_1000ul",
        8,
        label="1000 uL Tip Rack"
    )

    # ============================================================
    # INSTRUMENT
    # ============================================================
    # Your current OT-2 configuration: P1000 is on the LEFT mount.
    p1000 = protocol.load_instrument(
        "p1000_single_gen2",
        "left",
        tip_racks=[tips1000_8]
    )

    p1000.flow_rate.aspirate = ASPIRATE_RATE_UL_S
    p1000.flow_rate.dispense = DISPENSE_RATE_UL_S
    p1000.flow_rate.blow_out = BLOWOUT_RATE_UL_S
    p1000.default_speed = 80

    # ============================================================
    # SUBSTRATE SETUP — CAST AT ROOM TEMPERATURE FIRST
    # ============================================================
    hs4.open_labware_latch()
    protocol.pause(
        "Place a clean, flat aluminium foil on the Heater-Shaker/hot plate. "
        "Align the intended top-left corner of the usable foil area with the A1 reference position. "
        "Fix the foil edges, close the latch, then press RESUME. "
        "This protocol will cast droplets at 35 °C, then heat to 60 °C after all droplets are dispensed."
    )
    hs4.close_labware_latch()

    hs4.set_target_temperature(CASTING_TEMPERATURE_C)
    hs4.wait_for_temperature()

    slurry_source = rack20_6["A1"]

    # ============================================================
    # BUILD LOCAL COORDINATE SYSTEM
    # ============================================================
    a1 = electrode_plate["A1"].bottom(Z_HEIGHT_MM).point
    a2 = electrode_plate["A2"].bottom(Z_HEIGHT_MM).point
    b1 = electrode_plate["B1"].bottom(Z_HEIGHT_MM).point

    x_vec = Point(x=a2.x - a1.x, y=a2.y - a1.y, z=0)
    y_vec = Point(x=b1.x - a1.x, y=b1.y - a1.y, z=0)

    x_len = math.sqrt(x_vec.x**2 + x_vec.y**2)
    y_len = math.sqrt(y_vec.x**2 + y_vec.y**2)

    x_unit = Point(x=x_vec.x / x_len, y=x_vec.y / x_len, z=0)
    y_unit = Point(x=y_vec.x / y_len, y=y_vec.y / y_len, z=0)

    foil_origin = Point(
        x=a1.x + X_SHIFT_MM * x_unit.x + Y_SHIFT_MM * y_unit.x,
        y=a1.y + X_SHIFT_MM * x_unit.y + Y_SHIFT_MM * y_unit.y,
        z=a1.z
    )

    def local_point_to_location(local_x_mm, local_y_mm):
        point = Point(
            x=foil_origin.x + local_x_mm * x_unit.x + local_y_mm * y_unit.x,
            y=foil_origin.y + local_x_mm * x_unit.y + local_y_mm * y_unit.y,
            z=foil_origin.z
        )
        return Location(point, electrode_plate)

    # ============================================================
    # DEFINE DROPLET POSITIONS
    # ============================================================
    # Default layout for one replicate per volume on a 70 x 70 mm foil:
    # three spots in the upper row and two spots in the lower row.
    # This keeps large 150–200 µL droplets reasonably separated.
    if REPLICATES_PER_VOLUME != 1:
        raise ValueError(
            "This template is configured for 1 replicate per volume on a 65 x 50 mm foil. "
            "For triplicates, use three separate foils/runs or enlarge the foil area and manually define 15 positions."
        )

    positions_mm = [
        (FOIL_X_MM * 0.18, FOIL_Y_MM * 0.30),  # 75 µL
        (FOIL_X_MM * 0.50, FOIL_Y_MM * 0.30),  # 100 µL
        (FOIL_X_MM * 0.82, FOIL_Y_MM * 0.30),  # 125 µL
        (FOIL_X_MM * 0.34, FOIL_Y_MM * 0.72),  # 150 µL
        (FOIL_X_MM * 0.66, FOIL_Y_MM * 0.72),  # 200 µL
    ]

    droplet_targets = []
    for volume_ul, (x_mm, y_mm) in zip(DROPLET_VOLUMES_UL, positions_mm):
        droplet_targets.append((volume_ul, local_point_to_location(x_mm, y_mm), x_mm, y_mm))

    protocol.comment(
        f"Single-droplet test: {DROPLET_VOLUMES_UL} µL, one spot per electrode. "
        f"Casting at {CASTING_TEMPERATURE_C} °C, then drying at {DRYING_TEMPERATURE_C} °C until no visible solvent remains."
    )

    # ============================================================
    # RUN DROP-CASTING
    # ============================================================
    p1000.pick_up_tip()

    # Gentle pre-wet/resuspension step. Manually vortex/centrifuge-mix before starting if needed.
    for _ in range(3):
        p1000.aspirate(300, slurry_source.bottom(SOURCE_ASPIRATE_HEIGHT_MM), rate=1.0)
        protocol.delay(seconds=1)
        p1000.dispense(300, slurry_source.top(), rate=1.0)
        protocol.delay(seconds=1)

    for volume_ul, target, x_mm, y_mm in droplet_targets:
        protocol.comment(f"Dispensing {volume_ul} µL slurry at local position x={x_mm:.1f} mm, y={y_mm:.1f} mm.")

        p1000.aspirate(volume_ul, slurry_source.bottom(SOURCE_ASPIRATE_HEIGHT_MM), rate=1.0)
        protocol.delay(seconds=DELAY_AFTER_PIPETTING_S)

        p1000.dispense(volume_ul, target, rate=1.0)
        protocol.delay(seconds=DELAY_AFTER_PIPETTING_S)

        # Blow out at the droplet position to eject viscous residual slurry.
        # If this visibly distorts/splashes the droplet, set BLOWOUT_RATE_UL_S lower or comment out this line.
        p1000.blow_out(target)
        protocol.delay(seconds=DELAY_AFTER_PIPETTING_S)

    p1000.return_tip()

    # ============================================================
    # DRYING SEQUENCE
    # ============================================================
    protocol.comment(
        f"Drop-casting complete at {CASTING_TEMPERATURE_C} °C. "
        f"Heating to {DRYING_TEMPERATURE_C} °C for final drying."
    )

    hs4.set_target_temperature(DRYING_TEMPERATURE_C)
    hs4.wait_for_temperature()

    protocol.pause(
        f"The Heater-Shaker is now at {DRYING_TEMPERATURE_C} °C. "
        "Keep drying until no visible solvent remains on the electrodes. "
        "When the electrodes appear dry, press RESUME to stop heating."
    )

    hs4.deactivate_heater()

    protocol.comment(
        f"Single-droplet electrode drying completed. "
        f"Casting temperature: {CASTING_TEMPERATURE_C} °C; drying temperature: {DRYING_TEMPERATURE_C} °C."
    )